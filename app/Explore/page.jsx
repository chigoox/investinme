...
function page() {
    ...
    const PostInside = ({ type, link, creator, index }) => {
        ...
        return (<div className='h-full relative center-col overflow-hidden'>
            <PostView allPosts={data} index={index} showPostView={showPostView} setShowPostView={setShowPostView} currentPost={data[index].id} />
            <div className='absolute scale-75 top-0 -left-2 '>

                <UserAvatar user={creatorData.UserInfo} />
            </div>
            <button onClick={() => { setShowPostView(!showPostView) }} className='p-0 h-full w-full  overflow-hidden center-col bg-opacity-0'>
        ...
        <main className='bg-gradient-to-t via-black  from-black h-screen to-[#030012] flex min-h-screen w-screen  overflow-x-hidden flex-col items-center justify-center text-white'>
            <div className='flex flex-wrap  mb-12 trans md:px-20 lg:px-40 xl:px-32 py-4 w-full   h-full    lg:ml-60 overflow-y-scroll overflow-hidden  '>
                {data.map((postInfo, index) => {
                    ...
                    if (size == 1) return <Size1 key={postInfo.id} postInfo={postInfo} index={index} />
                    if (size == 2) return <Size2 key={postInfo.id} postInfo={postInfo} index={index} />
                    if (size == 3) return <Size3 key={postInfo.id} postInfo={postInfo} index={index} />
                    if (size == 4) return <Size4 key={postInfo.id} postInfo={postInfo} index={index} />
                    if (size == 5) return <Size5 key={postInfo.id} postInfo={postInfo} index={index} />
                    if (size == 6) return <Size6 key={postInfo.id} postInfo={postInfo} index={index} />
                })
                }
            </div>
